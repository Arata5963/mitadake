// app/javascript/controllers/post_edit_controller.js
// ==========================================
// 投稿編集コントローラー
// ==========================================
//
// 【このコントローラーの役割】
// 投稿編集画面で、動画選択 + アクションプラン入力 + サムネイル画像を
// 1つのフォームで処理する。post_create_controllerの編集版。
//
// 【処理フロー】
//
//   1. 既存の動画情報とアクションプランを初期表示
//      ↓
//   2. 必要に応じて動画を変更（YouTube検索）
//      ↓
//   3. アクションプランを編集（AI提案も可能）
//      ↓
//   4. サムネイル画像を変更（任意）
//      ↓
//   5. 送信 → Post & PostEntry を更新
//
...
export default class extends Controller {
  ...
  showUrlDetected(url, videoId) {
    ...
    this.resultsTarget.innerHTML = `
      <button type="button"
              ...
              data-action="click->post-edit#selectUrl"
  ...
  renderResults(videos) {
    ...
    this.resultsTarget.innerHTML = videos.map((video, index) => `
      <button type="button"
              ...
              data-action="click->post-edit#selectVideo"
  ...
  renderSuggestions(plans) {
    ...
    this.suggestionsContainerTarget.innerHTML = plans.map(plan => `
      <button type="button"
              data-action="click->post-edit#selectSuggestion"
