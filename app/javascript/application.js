...
// Turbo・Stimulusの読み込みとフラッシュメッセージ処理

import "@hotwired/turbo-rails"  // ページ遷移の高速化
import "controllers"            // Stimulusコントローラー

// sessionStorageからフラッシュメッセージを表示（達成処理等のリロード後に使用）
function showPendingFlash() {
  ...
  try {
    ...
    const isSuccess = type === 'notice' || type === 'success'
...
document.addEventListener('turbo:load', showPendingFlash)      // Turboページロード時
document.addEventListener('DOMContentLoaded', showPendingFlash) // 初回ロード時
