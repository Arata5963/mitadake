...
export default class extends Controller {
  ...
  static values = {
    ...
    updateUrl: String,                                         // 更新APIのURL
    currentPostId: Number,                                     // 現在の投稿ID
    currentVideoId: String,                                    // 現在の動画ID
    currentVideoTitle: String,                                 // 現在の動画タイトル
    currentVideoChannel: String,                               // 現在のチャンネル名
    currentVideoUrl: String,                                   // 現在の動画URL
    currentThumbnail: String,                                  // 現在のサムネイル
    hasCustomThumbnail: Boolean,                               // カスタムサムネイルがあるか
  ...
  connect() {
    ...
    this.thumbnailCleared = false                              // サムネイル削除フラグ
    this.videoChanged = false                                  // 動画変更フラグ
    this.selectedVideo = {
    ...
    if (this.hasInputTarget && this.currentVideoUrlValue) this.inputTarget.value = this.currentVideoUrlValue
  ...
  updateSelection(items) { items.forEach((item, i) => { item.style.background = i === this.selectedIndex ? "#f3f4f6" : "white" }) }
  ...
  showUrlDetected(url, videoId) {
    ...
    this.resultsTarget.innerHTML = `
      <button type="button" style="width: 100%; display: flex; align-items: center; gap: 12px; padding: 12px; background: white; border: none; text-align: left; cursor: pointer;"
              data-action="click->entry-edit#selectUrl" data-url="${escapeHtml(url)}" data-video-id="${videoId}" data-index="0"
        ...
        <div style="flex: 1;"><p style="font-size: 14px; font-weight: 500; color: #333; margin: 0;">この動画を選択</p><p style="font-size: 12px; color: #888; margin: 0;">クリックで選択</p></div>
        <svg style="width: 20px; height: 20px; color: #16a34a;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
      </button>`
  ...
  async fetchResults(query) {
    try {
      this.resultsTarget.innerHTML = `<div style="padding: 16px; text-align: center; color: #888; font-size: 14px;"><div style="display: inline-block; width: 16px; height: 16px; border: 2px solid #e0e0e0; border-top-color: #333; border-radius: 50%; animation: spin 1s linear infinite; margin-right: 8px;"></div>検索中...</div>`
      this.showResults()
    ...
    } catch (error) { console.error("YouTube search error:", error); this.resultsTarget.innerHTML = '<p style="text-align: center; color: #888; padding: 16px; font-size: 14px;">検索に失敗しました</p>' }
  }

  renderResults(videos) {
    if (videos.length === 0) { this.resultsTarget.innerHTML = '<p style="text-align: center; color: #888; padding: 16px; font-size: 14px;">動画が見つかりません</p>'; return }
    this.resultsTarget.innerHTML = videos.map((video, index) => `
      <button type="button" style="width: 100%; display: flex; align-items: flex-start; gap: 12px; padding: 12px; background: white; border: none; border-bottom: 1px solid #f3f4f6; text-align: left; cursor: pointer;"
              data-action="click->entry-edit#selectVideo" data-url="${video.youtube_url}" data-video-id="${extractVideoId(video.youtube_url)}"
        ...
        <div style="flex: 1; min-width: 0;"><p style="font-size: 14px; font-weight: 500; color: #333; margin: 0; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${escapeHtml(video.title)}</p><p style="font-size: 12px; color: #888; margin-top: 2px;">${escapeHtml(video.channel_name)}</p></div>
      </button>`).join("")
    this.selectedIndex = -1
  }
  ...
  setSelectedVideo(video) {
    ...
    this.selectedVideo = { videoId: video.videoId, title: video.title, channel: video.channel, url: video.url, thumbnail: video.thumbnail }
    this.hideResults()
  ...
  handleActionPlanInput() {
    ...
    if (this.hasPreviewTextTarget) this.previewTextTarget.textContent = text || 'アクションプランがここに表示されます'
  }
  ...
  async submitForm() {
    ...
    if (!this.selectedVideo) { alert('動画を選択してください'); return }
    if (!content) { alert('アクションプランを入力してください'); return }
    this.submitButtonTarget.disabled = true
    this.submitButtonTarget.textContent = '保存中...'
    try {
      let s3Key = null
      if (this.selectedFile) { this.submitButtonTarget.textContent = '画像アップロード中...'; s3Key = await uploadToS3(this.selectedFile) }
      this.submitButtonTarget.textContent = '保存中...'
      const thumbnailValue = this.thumbnailCleared ? "CLEAR" : s3Key
      const response = await fetchJson(this.updateUrlValue, {
        method: "PATCH",
        body: JSON.stringify({ post_entry: { content: content, thumbnail_s3_key: thumbnailValue, new_video_url: this.videoChanged ? this.selectedVideo.url : null } })
      })
      ...
      } else { alert(data.error || "更新に失敗しました"); this.submitButtonTarget.disabled = false; this.submitButtonTarget.textContent = '保存' }
    } catch (error) { console.error("Update error:", error); alert(error.message || "更新に失敗しました"); this.submitButtonTarget.disabled = false; this.submitButtonTarget.textContent = '保存' }
  }
}
